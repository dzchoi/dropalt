// log_module.h is included by log.h.
...
void log_write_always(unsigned level, const char* format, ...);

// Customized log_write() for LOG_* functions. See riot/core/lib/include/log.h.
#ifdef __cplusplus
#   define log_write(level, ...)  do { \
    if constexpr ( (level) <= LOCAL_LOG_LEVEL ) log_write_always((level), __VA_ARGS__); } while (0U)
    ...
    //         log_write_always(level, args...);
    // }
#else
#   define log_write(level, ...)  do { \
    if ( (level) <= LOCAL_LOG_LEVEL ) log_write_always((level), __VA_ARGS__); } while (0U)
#endif
